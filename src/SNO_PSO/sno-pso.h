...
inline void sno::init(population& curr_explorer, population& curr_miner, net& space_net, int& eval_count)
{
    ...
    velocity_s.assign(pop_size_s_init, solution(num_dims, bound_max));
    for (size_t i = 0; i < curr_explorer.size(); ++i) {
        for (size_t j = 0; j < curr_explorer[i].size(); ++j) {
            ...
            velocity_s[i][j] = bound_min + (bound_max - bound_min) * uniform_rand();
        }
    ...
    velocity_x.assign(pop_size_x_init, solution(num_dims, bound_max));
    for (size_t i = 0; i < curr_miner.size(); ++i) {
        for (size_t j = 0; j < curr_miner[i].size(); ++j) {
            ...
            velocity_x[i][j] = bound_min + (bound_max - bound_min) * uniform_rand();
        }
...
inline void sno::region_construct(net& space_net)
{
    ...
    // construct the net according the index of the elastic points
    int index = 0;
...
inline void sno::region_search(population& curr_explorer, population& curr_miner, net& space_net, int& eval_count) 
{
    ...
    vector<int> ept_rank = get_order_index(ept_value, [](pair<double, int> x, pair<double, int> y)->bool{ return x.first > y.first; });
    ...
    for (size_t i = 0; i < curr_explorer.size(); ++i) {
        ...
        for (int j = 0; j < num_dims; ++j) {
            velocity_s[i][j] = w * velocity_s[i][j] + c1 * uniform_rand() * (space_net.elastic_points[eps_index][j] - curr_explorer[i][j]);
            curr_explorer[i][j] += velocity_s[i][j];
            
            curr_explorer[i][j] = boundary_check(curr_explorer[i][j]);
        }
    ...
    population search_history = curr_explorer;
    for (size_t i = 0; i < search_history.size(); ++i) {
        space_net_adjustment(search_history[i], curr_obj_vals_s[i], curr_explorer, curr_miner, space_net, eval_count);
    }
...
inline bool sno::space_net_adjustment(const solution& sol, const double& obj_val, population& explorer, population& miner, net& space_net, int& eval_count)
{
    ...
    for (int i = 0; i < num_adjustment; ++i) {
        ...
        for (int j = 0; j < num_dims; ++j) {
            new_position[j] = space_net.elastic_points[ distance_order[i].second ][j] + c1 * uniform_rand() * (sol[j] - space_net.elastic_points[ distance_order[i].second ][j]);
...
inline void sno::point_search(population& curr_explorer, population& curr_miner, net& space_net, int& eval_count)
{
    ...
    for (size_t i = 0; i < curr_miner.size(); ++i) {
        ...
        for (int j = 0; j < num_dims; ++j) {
            velocity_x[curr_index][j] = w * velocity_x[curr_index][j] + c1 * uniform_rand() * (space_net.elastic_points[obj_vals_rank[0]][j]  - curr_miner[curr_index][j]);

            curr_miner[curr_index][j] += velocity_x[curr_index][j];
            curr_miner[curr_index][j] = boundary_check(curr_miner[curr_index][j]);
        }

        curr_obj_vals_x[curr_index] = evaluate(num_dims, curr_miner[curr_index]);
        if (++eval_count > num_evals)
            curr_obj_vals_x[curr_index] = numeric_limits<double>::max();

        if (curr_obj_vals_x[curr_index] < best_obj_val) {
            best_obj_val = curr_obj_vals_x[curr_index];
            best_sol = curr_miner[curr_index];
        }
        
        if (eval_count <= num_evals)
            avg_obj_val_eval[eval_count - 1] += best_obj_val;

        sorting_flag = space_net_adjustment(curr_miner[curr_index], curr_obj_vals_x[curr_index], curr_explorer, curr_miner, space_net, eval_count);
    }
}
...
inline void sno::population_adjustment(const net& space_net, population& curr_explorer, population& curr_miner, int& eval_count)
{
    ...
    while (curr_explorer.size() > static_cast<size_t>(new_size_s)) {
        ...
        velocity_s.erase(velocity_s.begin() + del_index);
    }
    ...
    while (curr_miner.size() < static_cast<size_t>(new_size_x)) {
        ...
        solution new_velocity(num_dims, bound_max);
        int rind = round(uniform_rand() * (curr_miner.size() - 1));
        // create a new solution with some information of elastic points
        for (size_t i = 0; i < new_sol.size(); ++i) {
            new_velocity[i] = (bound_max - bound_min) * uniform_rand() + bound_min;
            new_sol[i] = (bound_max - bound_min) * uniform_rand() + bound_min + w * velocity_x[rind][i]
		+ c1 * uniform_rand() * (space_net.elastic_points[obj_vals_rank[0]][i] - curr_miner[rind][i]);
        ...
        velocity_x.push_back(new_velocity);
    }
}
