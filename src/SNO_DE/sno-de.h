...
sno::RegionSearch(population& s, population& x, net& space_net, int& eval_count)
{
    ...
    population new_s(s.size(), solution(d, max_bound));
    vector<double> new_obj_vals_s(s.size(), numeric_limits<double>::max());
    for (size_t i = 0; i < s.size(); ++i) {
        // select a region
        ...
        // generate a new explorer according to the update strategy (Eqs. (4) and (14))
	const auto [r1, r2] = uniform_rand_pair(s.size(), i);
        const int j_rand = uniform_rand(d);
        for (int j = 0; j < d; ++j) {
            if (uniform_rand() < de_cr || j == j_rand) {
                const double r1v = s[r1][j];
                const double r2v = s[r2][j];
                new_s[i][j] = clamp_value(r1v + de_f * (space_net.p[p_sel][j] - r2v));
            }
            else {
                new_s[i][j] = s[i][j];
            }
        ...
        new_obj_vals_s[i] = evaluate(new_s[i], eval_count);
        if (eval_count <= MaxFES) {
	    if (new_obj_vals_s[i] < fbsf) {
		fbsf = new_obj_vals_s[i];
		bsf = new_s[i];
	    }
	    fbsf_avg_FES[eval_count - 1] += fbsf;
	}
    ...
    for (size_t i = 0; i < s.size(); ++i) {
        if (new_obj_vals_s[i] < fs[i]) {
            s[i] = new_s[i];
            fs[i] = new_obj_vals_s[i];
        }
    }

    // adjust the space net according to the history of region search
    population& search_history = new_s;
    for (size_t i = 0; i < search_history.size(); ++i)
        SpaceNetAdjustment(search_history[i], new_obj_vals_s[i], s, x, space_net, eval_count);
...
sno::PointSearch(population& s, population& x, net& space_net, int& eval_count)
{
	...
	const auto phi_x = uniform_rand(x.size());
	const auto [r1, r2] = uniform_rand_pair(x.size(), phi_x);
        const int j_rand = uniform_rand(d);
        for (int j = 0; j < d; ++j) {
            if (uniform_rand() < de_cr || j == j_rand) {
                const double r1v = x[r1][j];
                const double r2v = x[r2][j];
                new_sol[j] = clamp_value(r1v + de_f * (space_net.p[top][j] - r2v));
            }
            else {
                new_sol[j] = x[phi_x][j];
            }
	    ...
	    if (new_obj_val < fx[phi_x]) {
		fx[phi_x] = new_obj_val;
		x[phi_x] = new_sol;
	    }
	}

        if (SpaceNetAdjustment(new_sol, new_obj_val, s, x, space_net, eval_count))
...
sno::SpaceNetAdjustment(const solution& sol, const double obj_val, population& s, population& x, net& space_net, int& eval_count)
{
	...
	auto& k = distances[i].second;
        ...
        solution new_sol(d, max_bound);
	const auto [r1, r2] = uniform_rand_pair(s.size() + x.size());
        const int j_rand = uniform_rand(d);
        for (int j = 0; j < d; ++j) {
	    const int n = static_cast<int>(s.size());
            const double r1v = r1 >= n ? x[r1 - n][j] : s[r1][j];
            const double r2v = r2 >= n ? x[r2 - n][j] : s[r2][j];
            if (uniform_rand() < de_cr || j == j_rand)
                new_sol[j] = clamp_value(r1v + de_f * (sol[j] - r2v));
            else
                new_sol[j] = space_net.p[k][j];
        }
...
sno::PopulationAdjustment(population& s, population& x, const net& space_net, int& eval_count)
{
    ...
    while (s.size() > new_size_s) {
        auto del_index = distance(fs.begin(), max_element(fs.begin(), fs.end()));
    ...
    while (x.size() < new_size_x) {
        ...
        solution new_sol(d, max_bound);
	const auto [r1, r2] = uniform_rand_pair(x.size());
        for (size_t i = 0; i < new_sol.size(); ++i) {
            if (uniform_rand() < de_cr) {
                const double r1v = x[r1][i];
                const double r2v = x[r2][i];
                new_sol[i] = clamp_value(r1v + de_f * (space_net.p[top][i] - r2v));
	    }
            else {
                new_sol[i] = space_net.p[top][i];
	    }
        }
